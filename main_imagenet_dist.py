...
def generate_images_for_fid(pipe, label_count_path, save_dir='/home/ofekglick/BRECQ/generated_dataset'):
    ...
    for label, label_count in tqdm(dict_data.items()):
        ...
        if label == 0:
            continue
        for i in range(10):
            image = pipe(num_inference_steps=1, batch_size=batch_size, class_labels=[label])
            for j in range(len(image.images)):
                image.images[i].save(os.path.join(save_dir, f"imagenet64_label_{label}_{i * 10 + j}.png"))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print(args)
    # seed_all(args.seed)
    base_path = r'/home/ofekglick/BRECQ/'
