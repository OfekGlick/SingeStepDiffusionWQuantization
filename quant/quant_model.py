...
class QuantModel(nn.Module):
    ...
    def __init__(self, model: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}, ddim: bool = False):
        super().__init__()
    ...
    def forward(self, *args, **kwargs):
        if self.ddim:
            try:
                scaled_sample = args[0]
                t = args[1]
                class_labels = kwargs['class_labels']
                return_dict = kwargs['return_dict']
                return self.model(scaled_sample, timestep=t, class_labels=class_labels, return_dict=return_dict)
            except Exception:
                scaled_sample = args[0]
                try:
                    t = torch.tensor(1095.5067, dtype=torch.float32).cuda()
                except Exception:
                    t = torch.tensor(1095.5067, dtype=torch.float32)
                try:
                    random_class = torch.randint(0, 1000, (1,)).cuda()
                except Exception:
                    random_class = torch.randint(0, 1000, (1,))
                return self.model(scaled_sample, timestep=t, class_labels=random_class, return_dict=False)
        else:
            input = args[0]
